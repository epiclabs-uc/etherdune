...
uint16_t TCPSocket::write(uint16_t len, const byte* data)
{
	return buffer.write(len, data);
}

uint16_t TCPSocket::write(const String& s)
{
	return buffer.write(s.length(), (uint8_t*)s.c_str());
}

//uint16_t TCPSocket::write(const __FlashStringHelper *f)
//{
//	String s(f);
//	return write(s);
//}

uint16_t TCPSocket::write(const __FlashStringHelper* pattern, ...)
{
	char c;
	char buf[16];
	uint8_t i = 0;
